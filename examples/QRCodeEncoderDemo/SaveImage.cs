...
public partial class SaveImage : Form
	...
	private void OnLoad(object sender, EventArgs e)
		...
		QRCodeDimensionLabel.Text = Encoder.QRCodeDimension.ToString();
		ModuleSizeLabel.Text = Encoder.ModuleSize.ToString();
		QuietZoneLabel.Text = Encoder.QuietZone.ToString();
		QRImageDimensionLabel.Text = Encoder.QRCodeImageDimension.ToString();
		int BGImageSide = (int) (1.6 * Encoder.QRCodeImageDimension);
		...
		ErrorDiameterTextBox.Text = (Encoder.ModuleSize * 2).ToString();
	...
	private Bitmap CreateQRCodeImage
		...
		// image origin
		int ImageDimension = Encoder.QRCodeImageDimension;
		...
		int ModuleSize = Encoder.ModuleSize;

		// quiet zone 
		int QuietZone = Encoder.QuietZone;
		...
		int MatrixDimension = Encoder.QRCodeDimension;
		...
		for(int Row = 0; Row < MatrixDimension; Row++)
			...
			for(int Col = 0; Col < MatrixDimension; Col++)
				{
				// bar is black
				if(Encoder.QRCodeMatrix[Row, Col])
					{
	...
	private string SaveFileName()
		...
		Dialog.FileName = string.Format("QRCode{0}{1}.{2}",
			Encoder.ModuleSize.ToString(), Encoder.ErrorCorrection.ToString(), Format.ToString().ToLower());
